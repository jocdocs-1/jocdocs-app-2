...
export async function POST(req: Request) {
  try {
    ...
    const data = await resend.emails.send({
      ...
      to: email,
      subject: `${name ? `${name}, your` : "Your"} jocdocs Fan Ticket is ready`,
      html: `
        <div style="font-family: Arial, sans-serif; background-color: #ffffff; padding: 24px; text-align: center;">
          ...
          <h1 style="font-size: 26px; color: #000000; margin-bottom: 10px;">
            Your jocdocs Fan Ticket is ready.
          </h1>

          <!-- SUBTEXT -->
          <p style="font-size: 15px; color: #333333; margin-bottom: 30px;">
            Your Fan Ticket is now live. Use the links below anytime to view,
            share or update your ticket.
          </p>
    ...
    console.log("FAN TICKET EMAIL SENT:", data);
    ...
    console.error("FAN TICKET EMAIL ERROR:", error);

    return Response.json(
      { error: "Error sending Fan Ticket email" },
      { status: 500 }
    );
  }
}